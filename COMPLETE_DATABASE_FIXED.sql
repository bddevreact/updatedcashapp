-- BT COMMUNITY - COMPLETE DATABASE (FIXED VERSION)
-- Run this in Supabase SQL editor - handles existing constraints gracefully
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_user_level_trigger') THEN
...
-- =====================================================
-- SAMPLE DATA INSERTION (with ON CONFLICT)
-- =====================================================
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert own task completions' AND tablename = 'task_completions') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can read own task completions' AND tablename = 'task_completions') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can insert own special task submissions' AND tablename = 'special_task_submissions') THEN
    CREATE POLICY "Users can insert own special task submissions" ON special_task_submissions
      FOR INSERT WITH CHECK (user_id = get_telegram_user_id());
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can read own special task submissions' AND tablename = 'special_task_submissions') THEN
    CREATE POLICY "Users can read own special task submissions" ON special_task_submissions
      FOR SELECT USING (user_id = get_telegram_user_id());
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Public can read active task templates' AND tablename = 'task_templates') THEN
    CREATE POLICY "Public can read active task templates" ON task_templates
      FOR SELECT USING (is_active = true);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Public can read referral levels' AND tablename = 'referral_levels') THEN
